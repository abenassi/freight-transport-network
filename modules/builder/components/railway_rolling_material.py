...
class RollingMaterial():
    ...
    def add_freight_service(self, ton, distance):
        ...
        # correct negative rounding problems
        if ton > -0.001 and ton < 0:
            ton = 0

        # check there is no significant negative values
        msg = "Tons are negative! " + str(ton)
        assert ton >= 0, msg

        # add a service if ton is not 0
        if ton > 0:
    ...
    def remove_freight_service(self, ton, distance):
        """Increase the service requirements of rolling material for a service.
        ...
        # remove a service if ton is not 0
        if ton > 0:
